...
考研全流程 AI 学习教练，覆盖建档、答疑、错题归档与复习、今日/周计划、学习日志、周月复盘和模考校准。
...
│   ├── create_wrong_card.py     ← 新建错题卡（保留题干/选项，并强制完整解析落盘）
...
| `/reset [hard]` | 重置测试数据；默认保留基础建档信息，`hard` 彻底清空 |
...
| `/mock [科目] [题量]` | 对话式限时训练（alpha，启发式流程） |

概念解释、知识串联、解题挑错、Anki 卡片生成等直接用自然语言对话，无需专门指令。

说明：`/mock` 当前仍是 alpha 级对话流程，不是脚本化、可回归验证的结构化接口。
...
- **解析一次落盘**：`create_wrong_card.py` 会拒绝写出带 `待补充` 占位符的骨架卡
- **训练成绩可追踪**：`/progress` 可记录数学真题、408 套卷、英语阅读、政治选择等部分成绩，周/月复盘自动汇总趋势
- **初始化收口**：首次建档通过 `init_vault.py` 完成目录、模板和基础档案字段注入
- **可安全回滚测试**：`/reset` 默认清空测试痕迹但保留基础建档信息
- **周/月复盘闭环**：周计划、周/月复盘、模考分析都有脚本落地
- **防幻觉**：考频定性描述，禁止编造真题题号
- **LaTeX 兜底校验**：题干、选项和详解里的数学公式要求用 `$...$` / `$$...$$` 包裹，避免 Obsidian 渲染丢失
